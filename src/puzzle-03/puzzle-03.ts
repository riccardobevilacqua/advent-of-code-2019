...
export const stringToWire = (input: string): string[] => {
  return input.split(',');
};
...
export const calculateManhattanDistance = (): number => {
  const wire1: string[] = stringToWire(input1);
  const wire2: string[] = stringToWire(input2);
  return getManhattanIntersection(wire1, wire2);
}
